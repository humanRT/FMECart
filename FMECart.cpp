...
static int handleCommandLineArguments(int argc, char* argv[], App* app) {
    ...
    static const utils::getopt::option OPTIONS[] = { { "help", utils::getopt::no_argument, nullptr,
                                                         'h' },
...
static constexpr float4 sFullScreenTriangleVertices[3] = { { -1.0f, -1.0f, 1.0f, 1.0f },
    { 3.0f, -1.0f, 1.0f, 1.0f }, { -1.0f, 3.0f, 1.0f, 1.0f } };
...
int main(int argc, char** argv) {
    ...
    auto loadAsset = [&app](const utils::Path& filename) {
        ...
        for (Material* ma: materials) {
            ...
            ma->compile(Material::CompilerPriorityQueue::LOW,
                    UserVariantFilterBit::FOG | UserVariantFilterBit::SKINNING |
                            UserVariantFilterBit::SSR | UserVariantFilterBit::VSM);
#endif
        }
        ...
        app.viewer->setUiCallback([&app, scene, view, engine]() {
            ...
            if (ImGui::CollapsingHeader("Debug")) {
                ...
                if (*debug.getPropertyAddress<bool>("d.shadowmap.display_shadow_texture")) {
                    int layerCount;
                    int levelCount;
                    debug.getProperty("d.shadowmap.display_shadow_texture_layer_count",
                            &layerCount);
                    debug.getProperty("d.shadowmap.display_shadow_texture_level_count",
                            &levelCount);
                    ImGui::Indent();
                    ImGui::SliderFloat("scale",
                            debug.getPropertyAddress<float>(
                                    "d.shadowmap.display_shadow_texture_scale"),
                            0.0f, 8.0f);
                    ImGui::SliderFloat("contrast",
                            debug.getPropertyAddress<float>(
                                    "d.shadowmap.display_shadow_texture_power"),
                            0.0f, 2.0f);
                    ImGui::SliderInt("layer",
                            debug.getPropertyAddress<int>(
                                    "d.shadowmap.display_shadow_texture_layer"),
                            0, layerCount - 1);
                    ImGui::SliderInt("level",
                            debug.getPropertyAddress<int>(
                                    "d.shadowmap.display_shadow_texture_level"),
                            0, levelCount - 1);
                    ImGui::SliderInt("channel",
                            debug.getPropertyAddress<int>(
                                    "d.shadowmap.display_shadow_texture_channel"),
                            0, 3);
                    ImGui::Unindent();
                }
                ...
                if (dataSource.data) {
                    ImGuiExt::PlotLinesSeries(
                            ...
                            const_cast<void*>(dataSource.data), int(dataSource.count), 0, nullptr,
                            0.0f, 1.0f, { 0, 100 });
                }
#ifndef NDEBUG
    ...
    auto preRender = [&app](Engine* engine, View* view, Scene* scene, Renderer* renderer) {
        ...
        //        d));
        tcm.setTransform(root, mat4::translation(double3{ 2304097.1410110965, -4688442.9915525438,
                                                     -3639452.5611694567 } *
                                                 d));
